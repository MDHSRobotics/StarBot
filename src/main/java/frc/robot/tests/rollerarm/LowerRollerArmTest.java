...
package frc.robot.tests.rollerarm;
...
// Test class for the LowerRollerArm command.
public class LowerRollerArmTest implements TestRunnable {
    ...
    public int run(int iteration) {
        if (iteration == 0) {
            ...
            Logger.setup("Starting LowerRollerArmTest for " + TOTAL_SECONDS + " seconds...");
            m_timer.stop();
            m_timer.reset();
            m_timer.start();
            ...
            Logger.ending("Ending LowerRollerArmTest...");
            m_currentTime = 0.0;
            return 0;
        }
